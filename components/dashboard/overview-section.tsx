...
const getMetricIcon = (key: string) => {
  switch (key) {
    ...
    case "conversion_rate": return Percent;
...
export default function OverviewSection({ dashboardData, selectedPlatform, filteredProducts, setFilteredProducts }: Props) {
  ...
  const productColumns = useMemo<ColumnDef<ConsolidatedProduct>[]>(() => [
    { key: "name", header: "Product Name", sortable: true, render: (p: ConsolidatedProduct) => <div className="max-w-xs"><p className="truncate font-semibold text-white" title={p.name}>{p.name}</p></div> },
    { key: "status", header: "Status", sortable: true, align: "center" as const, render: (p: ConsolidatedProduct) => (
      <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-bold", p.status === "Active" ? "bg-emerald-950/30 text-emerald-400" : "bg-zinc-800 text-zinc-500")}>{p.status}</span>
    )},
  ...
  return (
    <div className="space-y-8 mt-4">
      ...
      <div>
        <h2 className="text-lg font-bold text-white mb-4">Scorecard Performance Sales</h2>
      ...
      <div>
        <h2 className="text-lg font-bold text-white mb-4">Scorecard Performance Ads</h2>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="flex items-center gap-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-5">
          ...
          <div><h3 className="text-xl font-bold text-white">18 Produk</h3><p className="text-sm font-semibold text-emerald-500">Performa Bagus</p></div>
        </div>
        <div className="flex items-center gap-4 rounded-xl border border-yellow-500/20 bg-yellow-500/10 p-5">
          ...
          <div><h3 className="text-xl font-bold text-white">24 Produk</h3><p className="text-sm font-semibold text-yellow-500">Performa Sedang</p></div>
        </div>
        <div className="flex items-center gap-4 rounded-xl border border-red-500/20 bg-red-500/10 p-5">
          ...
          <div><h3 className="text-xl font-bold text-white">5 Produk</h3><p className="text-sm font-semibold text-red-500">Performa Buruk</p></div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#1F1F23] bg-[#131316] p-6 shadow-sm">
        <div className="mb-6">
          <h3 className="text-lg font-bold text-white">Product Performance Tabel Pertumbuhan Produk</h3>
          <p className="text-sm text-[#8E8E95] mt-1">Audit pertumbuhan penjualan semua produk fashion wanita</p>
        </div>
        <DataTable
...
function RevenueDonutChart({ dashboardData }: { dashboardData: DashboardData }) {
  ...
  const shopeeGmv = allData.overview.find((m) => m.key === "gmv")?.value || 0;
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-[#1F1F23] bg-[#131316] p-6">
        <h3 className="text-lg font-bold text-white mb-4">Pangsa Pasar Revenue</h3>
        <div className="flex items-center gap-8">
          <div className="relative w-40 h-40">
            <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
              ...
              <circle cx="18" cy="18" r="15.5" fill="none" stroke="white" strokeWidth="3" strokeDasharray={`${tiktokPct} ${100 - tiktokPct}`} strokeDashoffset={`${-shopeePct}`} strokeLinecap="round" />
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#EE4D2D]" /><span className="text-sm text-white">Shopee</span><span className="text-sm font-bold text-white ml-auto">{shopeePct.toFixed(1)}%</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-white" /><span className="text-sm text-white">TikTok</span><span className="text-sm font-bold text-white ml-auto">{tiktokPct.toFixed(1)}%</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-emerald-500" /><span className="text-sm text-white">Website</span><span className="text-sm font-bold text-white ml-auto">{websitePct.toFixed(1)}%</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#1F1F23] bg-[#131316] p-6">
        <h3 className="text-lg font-bold text-white mb-4">Alokasi Spend Ads</h3>
        <div className="flex items-center gap-8">
          <div className="relative w-40 h-40">
            <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
              <circle cx="18" cy="18" r="15.5" fill="none" stroke="#1F1F23" strokeWidth="3" />
              ...
              <circle cx="18" cy="18" r="15.5" fill="none" stroke="white" strokeWidth="3" strokeDasharray={`${totalAdSpend > 0 ? (tiktokAd / totalAdSpend) * 100 : 0} ${100}`} strokeDashoffset={`${totalAdSpend > 0 ? -(shopeeAd / totalAdSpend) * 100 : 0}`} strokeLinecap="round" />
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#EE4D2D]" /><span className="text-sm text-white">Shopee Ads</span><span className="text-sm font-bold text-white ml-auto">{totalAdSpend > 0 ? ((shopeeAd / totalAdSpend) * 100).toFixed(1) : 0}%</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-white" /><span className="text-sm text-white">TikTok Ads</span><span className="text-sm font-bold text-white ml-auto">{totalAdSpend > 0 ? ((tiktokAd / totalAdSpend) * 100).toFixed(1) : 0}%</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#1877F2]" /><span className="text-sm text-white">Meta Ads</span><span className="text-sm font-bold text-white ml-auto">{totalAdSpend > 0 ? ((adOther / totalAdSpend) * 100).toFixed(1) : 0}%</span></div>
          </div>
        </div>
      </div>
    </div>
